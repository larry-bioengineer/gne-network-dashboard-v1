...
async function resetAllLocations() {
    ...
    if (!confirm('Are you sure you want to reset ALL locations? This will perform SSH reset operations on all devices in your configuration.')) {
        return;
    }
    
    // Show loading
    loading.style.display = 'block';
    loading.textContent = 'Resetting all locations... This may take a few minutes.';
    error.style.display = 'none';
    
    try {
        const response = await fetch('/api/network/reset_all_locations', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                timeout: 30  // 30 seconds timeout for each reset
            })
        });
        ...
        if (response.ok) {
            ...
            let message = `Reset All Operation Completed\n\nResult: ${data.message}\n\n`;
            
            if (data.data && data.data.successful_locations && data.data.successful_locations.length > 0) {
                message += `✅ Successful (${data.data.success_count}):\n`;
                data.data.successful_locations.forEach(location => {
                    message += `- ${location}\n`;
                });
                message += `\n`;
            }
            
            if (data.data && data.data.failed_locations && data.data.failed_locations.length > 0) {
                message += `❌ Failed (${data.data.failed_count}):\n`;
                data.data.failed_locations.forEach(failure => {
                    message += `- ${failure.location}: ${failure.error}\n`;
                });
                message += `\n`;
            }
            
            if (data.data) {
                message += `Total processed: ${data.data.total} locations\n`;
            }
            ...
            throw new Error(data.message || 'Reset all failed');
        }
        
    } catch (err) {
        console.error('Reset all error:', err);
        error.style.display = 'block';
        error.innerHTML = `<strong>Reset All Locations Failed:</strong><br>${err.message}`;
    } finally {
        loading.style.display = 'none';
    }
}