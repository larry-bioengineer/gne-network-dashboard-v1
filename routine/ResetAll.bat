@echo off
echo Starting Reset All Script...
echo.
...
:: Run the ResetAll.py script
echo.
echo Starting Reset All Script...
echo.

:: Use the virtual environment's Python to run the reset script
..\venv\Scripts\python.exe ResetAll.py

:: Check the exit code and display appropriate message
set script_result=%errorlevel%
if %script_result%==0 (
    echo.
    echo ✓ Reset All Script completed successfully!
) else if %script_result%==1 (
    echo.
    echo ❌ Reset All Script failed with errors!
) else if %script_result%==2 (
    echo.
    echo ⚠️ Reset All Script completed with some failures!
) else (
    echo.
    echo ❌ Reset All Script exited with code %script_result%
)
